...
### Concepts of analogue and digital signal, examples of their use
 ...
 - Some examples: 
	 ...
	 - traditional speedometer (the hands change with the pressure on the accelerator)
 ...
 - Digital
	- With digital signals we only use numbers, and we only extract the signal in a finite interval, such that we can properly, and efficiently process it for our needs(analog signal is basically infinite, and we of course cannot process infinity in a finite time). 
	- Digital signal always needs to be encoded and decoded between the sender, and receiver, because digital signal can only be efficiently read by computers, but digital data have to be encoded into electrical, or optical signal in order to be transported, and then decoded to be processed as digital signal again for computers.
...
### The digitalization of analogue signals
- 1. Sampling
	-  If an analogue signal needs to be converted into digital, it is done by so-called "sampling": at certain intervals (like, for sounds or images) a "sample" of the signal state is taken and stored digitally (from a pool of infinite amount of analog signal).
...
- Shannon formulated the law of how often a continuous signal should be sampled to recover the original signal from the sampled signal sequence. Based on Shannon's law, if we want the samples to properly represent the spectrums component with the largest frequency, the sampling frequency has to be at least twice the size of the spectrums component's with the largest frequency.
	- For Example, the human ear can hear up to 20 000 kHz, and the most common sampling frequency of audio signals(via microphone), is 44 000 kHz because it is at least twice as larger than 20 000khz, so it must represent the whole 20 000 kHz spectrum properly, so that it could be turned back into analog signal without issues.
---

### Digitization of sound, image and film
- Image digitalization
	- the visible image is essentially analogue information
	- the first step in the computer processing of images is the processing of the visual light of an image into digits (digitization). This is done by scanners and digital cameras
- Types
	...
	- Grayscale scanning: corresponds to a shade of gray depending on the intensity of the reflected light. The color of a point can be assigned from 0 to 255 (256 shades of gray).
	- Color scanning: the color filters of the sensors breaks down the reflected "mixed color" light into a mix of red (R), green (G) and blue (B) values, so that the RGB values transmitted to the computer shows a single color. There is a set of 255 shades for each red green blue color (255<sup>3</sup> ≈ 16 million colors).
	- The quality of the digitized image also depends on the resolution of the scanner, measured in DPI (dots per inch, or the number of pixels per inch).
		- hardware (real) resolution: what the scanner is capable of through its optical system
		- software resolution (interpolated) : increasing the number of pixels by calculation, which doesn't enhance the image

-  The digital camera also produces a raster image.
	- by changing setting the exposure time, how long the lens is opened, and how long the camera's light censor allowed to receive light, the light sensor's millions of tiny light sensitive cells detect light
	- before the light sensor, with different filters usually 50% green, 25% red and 25% blue the image is altered
	- with the help of these filters and light sensor, each cell's value is digitized, saved, and stored into a series of RGB values, with the place of each pixel within the matrix also saved
	- the resolution of an image is called megapixel (product of vertical, and horizontal lines i.e. 1920 by 1080)
---
### Sound digitization
- steps are very similar to other analog signal digitization
- sound also has to be encoded and decoded when transporting the data between computers
- encoding of sound signal is done by an ADC module (analog to digital converter) 
...
- some conversions for reference
	- 1 GB = 8 Gbit = 1 000 MB = 8 000 Mbit = 1 000 000 kB = 8 000 000 kbit
	- 1 GiB = 1024 MiB = 1 048 576 KiB
...
- representing integer numbers (on 1 byte, 8 bits):
	...
	- Integers in binary are represented as 2<sup>1</sup>, 2<sup>2</sup>, 2<sup>3</sup>, 2<sup>4</sup>, etc.
		- for example starting from 2<sup>0</sup>, all the  0 1 0 0 1 1 1 1 equals to 79 as 1 + 2 + 4 + 8 + 64 = 79 (from right to left, counting each 1, way to 2<sup>7</sup>)
...
- types:
	- reversible: when you can convert the date back and forth.
	- Irreversible: when you cannot convert it back and forth.
- examples of code systems/sets: morse, punch card, digitizing sounds or images
- character sets:
...
- Raster illustration:
	...
	- The color of each pixel has to be stored. The color quality of an image is determined by its color depth, i.e. how many colors are selected from a palette of colors. The color of the pixels.
	- When saving, information about the image in different formats (BMP, GIF, JPG, etc.), a lot of different things are written to the header of the file including color depth, so the actual space occupied is slightly larger than the actual size of the image.
	- 4 bit: 16 colors, 8 bit: 256 colors, etc.
	- The real solution is the RGB color model, with 16 million colors. 
- vector image:
	...
	- Its advantage is small space requirements, and flexible scaling, without any quality loss (since it isn't defined in a fixed coordinate system)
...
- RGB 
	- In the RGB color system, colors are created by adding the three primary colors red, green, blue. This is an additive color mixing. This type of color mixing system is based on the emitted or perceived light and can only be created by light-emitting devices (e.g. monitors). If all three colors are projected in "maximum quantity", white light is displayed, otherwise black.
	- 3 bytes are used, 1 for each color, which sets how much of each color should be added (i.e.: 255 255 255 is pure white)
	- 3 bytes are 24 bit, so there's 16 million+ colors possible with this system
- CMYK:
	- When printing colored images, the CMYK model is used. Unlike RGB, this is not additive but subtractive color mixing.
	- Consists of cyan, purple, yellow, and black
	- it consist of 4 bytes, which is 32 bit 
